        ...
        <div class="player-card ${p.eliminated ? 'eliminated' : ''}">
          ...
          <div class="ct-balls-row">
            ${[...ctState.distribution.groups[i]].map(b => {
                const ballState = ctState.balls[b]; // 'in' | 'out' | undefined (retirée)
                if (ballState === undefined) return ''; // bille retirée, non trackée
                return `
                <button class="ct-ball-btn ${ballState === 'out' ? 'pocketed' : ''}"
                        onclick="ctToggleBall(${b})"
                        ${ctState.balls[b] === 'out' ? 'disabled' : ''}
                        title="Bille ${b}">
...
function ctToggleBall(ballNum) {
    if (ctState.balls[ballNum] === 'out') return;
    ctPushHistory();
    ctState.balls[ballNum] = 'out';
...
function ctOpenFaultMenu() {
    ...
    list.innerHTML = ctState.players
        .map((p, i) => ({ ...p, realIndex: i }))
        .filter(p => !p.eliminated)
        .map(p => `
            <button class="joker-choice-btn" onclick="ctSelectFaulter(${p.realIndex})">
                ${EMOJIS[p.realIndex % EMOJIS.length]} ${p.name}
            </button>
        `).join('');
